...
CREATE TABLE processes (
    ...
    CONSTRAINT fk_processes_company FOREIGN KEY (company_id) REFERENCES companies(id) ON DELETE RESTRICT,
    CONSTRAINT fk_processes_client FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE RESTRICT,
    CONSTRAINT fk_processes_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT
);

CREATE INDEX idx_clients_company_name ON clients (company_id, full_name);
CREATE INDEX idx_clients_company_cpf ON clients (company_id, cpf);
...
CREATE INDEX idx_processes_external_id ON processes (company_id, external_id);
