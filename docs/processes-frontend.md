...
└── interfaces/
    ...
    └── document.interface.ts       # Contrato de tipos para documentos
```
...
* **Funcionalidades**:
  * **Clientes Associados em Grade Responsiva**: Apresentação da lista completa de clientes em um layout de grade moderno (`grid grid-cols-1 sm:grid-cols-2 gap-4`), exibindo apenas o nome completo do cliente, a Cidade/Estado do estabelecimento associado, o botão **Ver Perfil do Cliente** (que abre de forma contextualizada a `ClientDetailModal` contendo as informações cadastrais e de contato do cliente, com botões para cópia rápida com feedback de "Copiado!" ao lado dos campos de E-mail, Data de Nascimento, CPF, RG e CNH), e o botão do **Cofre de Segredos**.
  ...
  * **Sistema de Abas Dinâmico no Rodapé:** A parte inferior foi refatorada de forma rica para incluir duas abas HSL interativas ("Anotações de Acompanhamento" e "Documentos do Processo"). O conteúdo alterna de forma fluida através do estado local `activeTab`.
    *   **Aba Anotações:** Painel de histórico cronológico reativo, contendo listagem de notas e formulário de adição rápida. Exibição dinâmica de badges de visibilidade e cronômetro em tempo real limitando modificações a 15 minutos.
    *   **Aba Documentos:** Tabela com suporte a download físico seguro, visualização adaptativa de arquivos diretamente no navegador e controle de deleção lógica via lixeira do storage.
...
### 3.10. `ProcessDocuments.tsx`
* **Responsabilidade**: Componente contêiner principal da aba de documentos. Orquestra a tabela com colunas de Nome, Descrição, Data de Upload e Tamanho do arquivo, além de gerenciar todos os modais de ação correspondentes.

### 3.11. `DocumentUploadModal.tsx`
* **Responsabilidade**: Modal interativo que implementa uma área de *Drag and Drop* rica com contornos tracejados transitando sob eventos do mouse. Extrai o nome padrão a partir do arquivo físico adicionado e realiza validações de tamanho (máximo 10MB) e de extensões permitidas (.pdf, .png, .jpg, .jpeg, .docx, .xlsx).

### 3.12. `DocumentEditModal.tsx`
* **Responsabilidade**: Formulário de alteração de nome, descrição e substituição opcional de arquivo binário físico.

### 3.13. `DocumentSafetyConfirmModal.tsx`
* **Responsabilidade**: Modal secundário de dupla confirmação exigido quando o usuário opta por substituir o arquivo físico do documento. O botão de salvar permanece inativo até a digitação minúscula da frase `"alterar documento"`.

### 3.14. `DocumentDeleteModal.tsx`
* **Responsabilidade**: Modal de aviso alertando a deleção física da tabela e movimentação transacional do arquivo para a lixeira do storage.

### 3.15. `DocumentViewModal.tsx`
* **Responsabilidade**: Visualizador inteligente. Se o arquivo for PDF ou Imagem, realiza uma chamada segura com JWT e streama o arquivo como `Blob`, criando um ObjectURL temporário para renderizá-lo embutido no navegador sem expor URLs desprotegidas. Para Word/Excel, sugere e facilita o download direto.
...
* **useProcessDocuments**: Gerencia o estado de metadados de arquivos, expondo funções reativas de listagem (`fetchDocuments`), upload (`uploadDocument`), edição (`updateDocument`), exclusão (`deleteDocument`) e downloads programáticos seguros com criação e expurgo de ObjectURLs.

### 4.2. Serviços `process.service.ts`, `establishment.service.ts`, `annotationService.ts` e `documentService.ts`
...
* O `documentService.ts` implementa requisições complexas de envio e atualização usando cabeçalhos `multipart/form-data` e downloads streamados de arquivos como `responseType: 'blob'`.
