...
*   `internal/storage/local.go`: Implementação física do driver de storage em disco local, gravando no caminho dinâmico configurado por `LOCAL_STORAGE_PATH` (com fallback a `./storage`) e gerenciando arquivos `.tag` para meta-tags de lixeira.
